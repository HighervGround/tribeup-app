// Enhanced Auth component using official Supabase components
export { default as Auth } from './EnhancedAuth';

// Legacy Auth component (backup available as Auth.backup.tsx)
// This file now exports the enhanced version to prevent orphaned users
...
function Auth() {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'apple') => {
    console.log(`🔐 Auth Component - ${provider} button clicked`);
    setLoading(true);
    setError('');
    
    // Set a timeout to reset loading state if OAuth doesn't complete
    const timeoutId = setTimeout(() => {
      console.log(`⏰ OAuth timeout reached for ${provider}`);
      setLoading(false);
    ...
    try {
      ...
      console.log(`✅ signInWithOAuth completed for ${provider}`);
      ...
      if (error.message?.includes('popup')) {
        ...
        setError('OAuth provider not configured properly. Please contact support.');
      } else {
