...
-- Games table indexes for common query patterns (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'games') THEN
        ...
        -- Simple indexes without date predicates to avoid IMMUTABLE function issues
        CREATE INDEX IF NOT EXISTS idx_games_date_only ON games(date);
        CREATE INDEX IF NOT EXISTS idx_games_location_date ON games(location, date);
    END IF;
END $$;

-- User profiles indexes for user lookups (only if table exists)
DO $$
BEGIN
